...
unsigned int  BindHttpAffordanceHandler::ParseThingsURI(std::string RxURI,std::string* ArrayOfURI)
{
...
void BindHttpAffordanceHandler::handleRequest(HTTPServerRequest &req, HTTPServerResponse &resp)
{
    ...
    else
    {
        ...
        if (ThingURI[URI_AFFORDANCES_INDEX] == "properties")
        {
            Interaction_Type = PROPERTY_INTERACT;
        }
        else if (ThingURI[URI_AFFORDANCES_INDEX] == "actions")
        {
            Interaction_Type = ACTION_INTERACT;
        }
        else if (ThingURI[URI_AFFORDANCES_INDEX] == "events")
        {
